...
class WCSS_Shortcodes {
	...
	public static function init() {
		$shortcodes = array(
			...
			'subscription_status_hca'      => 'get_subscription_status_hca',
		);
    ...
    /**
     * This Harp Column Academy-specific function returns an HTML string for four cases:
     * Not logged in -- show login link
     * Logged in but no subscription: Prompt to subscribe
     * Logged in with active subscription(s): Check subscription SKU, 
     * if it starts with HCA-A show subscription name and expiration date and 
     * link to my-account/subscriptions
     * Logged in with expired subscription(s): If SKU starts with HCA-A show name and prompt to renew.
     *
    ...
    public static function get_subscription_status_hca( $atts ) {
	global $wpdb, $post;
	...
	// got some subscriptions. If there's an active one that matches "HCA-A", return it
	foreach ( $subscriptions as $index => $subscription ) {
	    if ( $subscription->has_status( 'active' ) && preg_match('^HCA-A', $subscription->get_sku()) === true ) {
	...
	foreach ( $subscriptions as $index => $subscription ) {
	    if ( $subscription->has_status( 'expired' ) && preg_match('^HCA-A', $subscription->get_sku()) === true ) {
	...
	// No subscriptions at all ever.
	
	echo 'Hello ' . $user->first_name
	   . '! | <a href="/academy/get-started">Get Started With HCA</a>';
	return ob_get_clean();

    } // END get_subscription_status_hca()
